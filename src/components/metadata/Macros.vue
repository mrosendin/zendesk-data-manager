<template>
  <div id="macros">
    ...
    <search type="macros" :onFetch="onFetch"/>
...
export default {
  ...
  mounted () {
    client.request('/api/v2/macros.json').then(data => {
      format(data.macros, 'macros', this.columns).then(results => {
        this.results = results
