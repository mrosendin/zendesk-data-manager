...
export default {
  ...
  methods: {
      ...
      ids.forEach(id => {
        client
          ...
          .then(() => {
            ...
            if (count > 1)
              this.messages.success = `${count} organizations have been deleted.`;
            else
              this.messages.success = `${count} organization has been deleted.`;
          });
      });
    },
    ...
    onChange(value, name) {
      ...
      client
        .request(`/api/v2/autocomplete/tags.json?name=${value}`)
        .then(response => {
  ...
  mounted() {
    ...
    client
      .request(url)
      .then(data => {
        format(data.organizations, "organizations", this.columns).then(
          results => {
            this.results = results;
  ...
  beforeCreate() {
    client
      .request("/api/v2/organization_fields.json")
      .then(data => {
